...
std::vector< thrust::device_vector<double> > GaussianMutator::apply(std::vector< thrust::device_vector<double> > &individuals) {
    ...
    std::vector< thrust::device_vector<double> > toRet(1, newInd);
    return toRet;
}